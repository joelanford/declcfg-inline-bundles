...
func WalkFS(root fs.FS, f func(path string, cfg *DeclarativeConfig, err error) error) error {
	if root == nil {
	...
	if err := walkFiles(root, func(path string, r io.Reader) error {
		...
		cfg, err := readYAMLOrJSON(r)
		if err != nil {
			return f(path, cfg, err)
		}
		if err := readBundleObjects(cfg.Bundles, root, path); err != nil {
			return f(path, cfg, err)
		}
		return f(path, cfg, nil)
	}); err != nil {
